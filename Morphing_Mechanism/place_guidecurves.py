...
def main(input_file):
    ...
    for i in range(dist.shape[0]):
        ...
        for j in range(fx.shape[0]):
            ...
            x0.append(x_new[0]); xm.append(x_new[1]); x1.append(x_new[2])
            y0.append(y_new[0]); ym.append(y_new[1]); y1.append(y_new[2])
            z0.append(z_new[0]); zm.append(z_new[1]); z1.append(z_new[2])
        
    # turn into np arryas, and save to xd lists
    x0 = np.array(x0); x1 = np.array(x1); xm = np.array(xm)
    y0 = np.array(y0); y1 = np.array(y1); ym = np.array(ym)
    z0 = np.array(z0); z1 = np.array(z1); zm = np.array(zm)

    xd.append(x0); xd.append(xm); xd.append(x1)
    yd.append(y0); yd.append(ym); yd.append(y1)
    zd.append(z0); zd.append(zm); zd.append(z1)
