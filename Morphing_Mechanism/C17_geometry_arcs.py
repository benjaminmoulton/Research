...
def simplify(vals,run):
    ...
    dicti["n"]  = vals[run]["num arcs"]
    dicti["atr"] = vals[run]["arcs to remove"]
    dicti["charc"] = vals[run]["chamfer arcs"]
    dicti["np"]  = vals[run]["num arc points"]
    dicti["at"]  = vals[run]["arc type"]
    dicti["ac"] = vals[run]["arc concave"]
    dicti["ath"]  = vals[run]["arc thickness [mm]"] / dicti["c"] / in2mm
...
def arcs(xy,info,shift=0.0):
    ...
    for i in range(info["n"]): # 2): # 
        ...
        if info["charc"]:
            ...
            xbzt,ybzt = Bezier_Cubic(x_bez,y_bez)
            ...
            xbzb,ybzb = Bezier_Cubic(x_bez,y_bez)
...
def remove_arcs(x,y,info):
    ...
    for i in range(len(info["atr"])):
        if info["atr"][i] < 1:
            ...
            raise ValueError("ARC to remove value {}".formate(info["atr"][i])\
                + " is not an integer")
...
def find_wall(O,foil,info):
    ...
    info["tw"] = find_thickness_x(foil,thick,xl,xu)
...
def te_hole(O,I,it,foil,info):
    ...
    # input array values
    x_hole[2] = radius * np.cos(theta) + x_center
    y_hole[2] = radius * np.sin(theta) + y_center
...
def make_polygon(x,y,vals):
    ...
    n = x["hol"].shape[0] + 2 + (vals["num arcs"]-len(vals["arcs to remove"]))
    # initialize arrays
    ...
    arcs = np.linspace(1,vals["num arcs"],vals["num arcs"]).astype(int) - 1
    arem = np.array(vals["arcs to remove"]) - 1
    ...
    for i in range(n-j-1):
        ...
        for k in range(rfli,arkpi,-1):
            ...
            if k > arkpi+1:
                x_svg[j] = np.append(x_svg[j],x["arc"][0][k-1][:-1])
                y_svg[j] = np.append(y_svg[j],y["arc"][0][k-1][:-1])
        
        # add side arc
        x_svg[j] = np.append(x_svg[j],x["arc"][1][arkpi][:-1])
        y_svg[j] = np.append(y_svg[j],y["arc"][1][arkpi][:-1])
        ...
        for k in range(arkpi+1,rfli+1):
            ...
            if k < rfli:
                x_svg[j] = np.append(x_svg[j],x["arc"][1][k][:-1])
                y_svg[j] = np.append(y_svg[j],y["arc"][1][k][:-1])

        # add side arc, or end cap depending on if it is the last
        if rfli == vals["num arcs"]:
            x_svg[j] = np.append(x_svg[j],x["rfl"][1][0][:-1])
            y_svg[j] = np.append(y_svg[j],y["rfl"][1][0][:-1])
        else:
            x_svg[j] = np.append(x_svg[j],x["arc"][0][rfli][:-1])
            y_svg[j] = np.append(y_svg[j],y["arc"][0][rfli][:-1])
    ...
    for k in range(rfli+1):
        ...
        if k < rfli-1:
            x_svg[j] = np.append(x_svg[j],np.flip(x["arc"][0][k])[:-1])
            y_svg[j] = np.append(y_svg[j],np.flip(y["arc"][0][k])[:-1])
    ...
    if len(arkp) == 0:
        ...
        # add side arc
        x_svg[j] = np.append(x_svg[j],np.flip(x["arc"][0][arkpi])[:-1])
        y_svg[j] = np.append(y_svg[j],np.flip(y["arc"][0][arkpi])[:-1])
    ...
    for k in range(rfli,0,-1):
        ...
        x_svg[j] = np.append(x_svg[j],np.flip(x["arc"][1][k-1])[:-1])
        y_svg[j] = np.append(y_svg[j],np.flip(y["arc"][1][k-1])[:-1])
...
def main(jsonfile,run="C17"):
    ...
    for k in range(len(df)):
        ...
        xsvg,ysvg = make_svg(u,v,vals[run],info,filename)
