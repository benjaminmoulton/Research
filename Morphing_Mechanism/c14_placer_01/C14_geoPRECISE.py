...
def split(x,y):
    ...
    num = x["ton"].shape[0] + x["mou"].shape[0] + x["out"].shape[0] + \
        int(x["rfl"].shape[0]*x["rfl"].shape[1]-1) + \
             int(x["arc"].shape[0]*x["arc"].shape[1]) + 1
    ...
    # pass in roofs and floors
    for i in range(x["rfl"].shape[0]):
        for k in range(x["rfl"].shape[1]):
            if type(x["rfl"][i][k]) != float and type(x["rfl"][i][k]) != int:
                # pass in values
                xsplit[j] = x["rfl"][i][k]
                ysplit[j] = y["rfl"][i][k]
                # increase counter
    ...
    # pass in arc tops and bottoms
    for i in range(x["arc"].shape[1]):
        # pass in values
        xsplit[j] = np.array([x["arc"][0][i][ 0],x["arc"][1][i][ 0]])
        ysplit[j] = np.array([y["arc"][0][i][ 0],y["arc"][1][i][ 0]])
        # increase counter
        j += 1
        # pass in values
        xsplit[j] = np.array([x["arc"][0][i][-1],x["arc"][1][i][-1]])
        ysplit[j] = np.array([y["arc"][0][i][-1],y["arc"][1][i][-1]])
        # increase counter
        j += 1
...
def guide_curves(x,y,I,vals,info):
    ...
    # add rfl points that will be used as guide curves
    end_grp0 = ["ton","ton","out"]
    end_pt0 = [(2,0),(2,-1),(-1,-1)]
    # create lists of rfl points
    rfl_grp0 = []; rfl_pt0 = []
    i = 0
    for j in range(x["rfl"].shape[1]):
        if type(x["rfl"][i][j]) != int and type(x["rfl"][i][j]) != float:
            rfl_grp0.append("rfl")
            rfl_pt0.append((((i,j),0)))
            rfl_grp0.append("rfl")
            rfl_pt0.append((((i,j),-1)))
    i = 1
    for j in range(x["rfl"].shape[1]-1,-1,-1):
        if type(x["rfl"][i][j]) != int and type(x["rfl"][i][j]) != float:
            rfl_grp0.append("rfl")
            rfl_pt0.append((((i,j),-1)))
            rfl_grp0.append("rfl")
    ...
    # add to 2d lists
    grp[0] = grp[0] + rfl_grp0 + end_grp0
    point[0] = point[0] + rfl_pt0 + end_pt0
