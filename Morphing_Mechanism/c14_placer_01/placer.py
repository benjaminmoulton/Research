...
def make_shapes(dist,start,vals):
    ...
    c14_guides = []; af_guides = []; arc_guides = []
    ax = []; ay = []; az = []
    ...
    for i in range(start,dist.shape[0]): # start+33,dist.shape[0]): # 
        ...
        try:
            ...
            # translate arc guides
            # mirror across x axis
            arcsgc["arcs gc"][:,:,0] *= -1

            # flip iy and iz
            ia = arcsgc["arcs gc"][:,:,1] * -1.
            arcsgc["arcs gc"][:,:,1] = arcsgc["arcs gc"][:,:,2] * 1.
            arcsgc["arcs gc"][:,:,2] = ia * 1.

            # rotate by the dihedral
            ry = arcsgc["arcs gc"][:,:,1]*np.cos(dist[i,4]) + \
                arcsgc["arcs gc"][:,:,2]*np.sin(dist[i,4])
            rz = arcsgc["arcs gc"][:,:,2]*np.cos(dist[i,4]) - \
                arcsgc["arcs gc"][:,:,1]*np.sin(dist[i,4])
            arcsgc["arcs gc"][:,:,2] = rz; arcsgc["arcs gc"][:,:,1] = ry

            # shift to the c/4 point
            arcsgc["arcs gc"][:,:,0] += dist[i,0]
            arcsgc["arcs gc"][:,:,1] += dist[i,1]
            arcsgc["arcs gc"][:,:,2] += dist[i,2]
            
            # add to list
            arc_guides.append(arcsgc["arcs gc"])

            # initialize arx and ary
            arx = arcsgc["x arcs"]; ary = arcsgc["y arcs"]; arz = 0.*ary
        
        ### determine 2D values of the shape
        # mirror airfoil across y axis
        ix2d = -1. * ix; isx2d = -1. * isx; arx2d = -1. * arx
        ...
        # mirror across x axis
        ix3d = -1. * ix; isx3d = -1. * isx; arx3d = -1. * arx

        # flip iy and iz
        ia3d = iy * -1.; iy3d = iz * 1.; iz3d = ia3d * 1.
        isa3d = isy * -1.; isy3d = isz * 1.; isz3d = isa3d * 1.
        ara3d = ary * -1.; ary3d = arz * 1.; arz3d = ara3d * 1.
        ...
        rry3d = ary3d*np.cos(dist[index,4]) + arz3d*np.sin(dist[index,4])
        rrz3d = arz3d*np.cos(dist[index,4]) - ary3d*np.sin(dist[index,4])
        arz3d = rrz3d; ary3d = rry3d

        # shift to the c/4 point
        ix3d += dist[i,0]; isx3d += dist[index,0]; arx3d += dist[index,0]
        iy3d += dist[i,1]; isy3d += dist[index,1]; ary3d += dist[index,1]
        iz3d += dist[i,2]; isz3d += dist[index,2]; arz3d += dist[index,2]
        ...
        # write to dxf file if first, last, or first airfoil
        ...
        if not (type(isx) == np.float64 and type(isy) == np.float64)\
            ...
            nxt = " arc"
            dxf.dxf(file_location+nxt,arx,ary,arz,file_info,geometry="spline")
            if i == index_saf:
        ...
        px = ix; py = iy; pz = iz
        psx = isx; psy = isy; psz = isz
        prx = arx; pry = ary; prz = arz
    ...
    return ax,ay,az,c14_guides,af_guides,arc_guides
...
def arc_guides_dxf(arc_guides,vals):
    # NOTE: 
    # i -- plane #
    # j -- hole #
    # k -- guide curve
    ...
    for j in range(arc_guides[0].shape[0]):
        # initialize extruder guide curves
        x_dxf = np.zeros((arc_guides[0][j].shape[0],),dtype=np.ndarray)
        y_dxf = np.zeros((arc_guides[0][j].shape[0],),dtype=np.ndarray)
        z_dxf = np.zeros((arc_guides[0][j].shape[0],),dtype=np.ndarray)
...
def main(input_file):
    ...
    # create 3d, 2d, and guidecurves
    ax,ay,az,c14_guides,af_guides,arc_guides = make_shapes(dist,start,vals)

    # create c14 guidecurves dxf file
    c14_guides_dxf(c14_guides,vals)

    # create arc guidecurves dxf file
    arc_guides_dxf(arc_guides,vals)
