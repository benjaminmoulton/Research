...
def main(jsonfile):
    ...
    if "arcs start [x/c]" in vals["C14"] and "arcs end [x/c]" in vals["C14"]:
        info["a0"] = vals["C14"]["arcs start [x/c]"]
        info["a1"] = vals["C14"]["arcs end [x/c]"]
        I["I"]["arcdx"] = (info["a1"] - info["t"] - info["a0"]) / (info["n"]+1)
    else:
    ...
    print(info["f"],I["O"]["t"](info["f"]),I["O"]["b"](info["f"]))
    ...
    for k in range(len(df)):
        ...
        if vals["C14"]["guide curve return"]:
            ...
            if vals["C14"]["actuation hole return"]:
                actuate_hole(x,y,I,vals,info,splitdict)
            ...
            for group in u:
                ...
                if group == "arc": 
                    ...
                    for i in range(u[group].shape[0]):
                        ...
                        # if group == "out":
                        #     if i in [0,u[group].shape[0]-1,u[group].shape[0]-2]:
                        #         c = "b"
                        #     else:
                        #         c = "k"
                        ...
                        #     else:
                        #         d = 51
                        #         plt.plot(u[group][i][:d],v[group][i][:d],"b",label=l)
                        #         plt.plot(u[group][i][d-1:],v[group][i][d-1:],"k",label=l)
    ...
    if vals["C14"]["show plot"]:
        if vals["C14"]["show legend"]:
            plt.legend()#loc=(11.011,0.214204))
        

        # # read in file
        # # filename = "/home/ben/Desktop/arcs_p15_dig.txt"
        # filename = "/home/ben/Desktop/kincs_m10_dig.txt"
        ...
        #         x.append(float(line.split()[0])*info["c"])
        #         y.append(float(line.split()[1])*info["c"])
        ...
        # # changes
        # # da = 2.0; dx = 0.0; dy = 0.034372;scale = False # arcs_m05 -5.0 deg
        # # da = 3.0; dx = 0.0; dy = 0.061257;scale = False # arcs_m10 -8.5 deg
        # # da = 3.0; dx = 0.0; dy = 0.060575; scale = True # arcs_m15 -11.5 deg
        # # da = 3.0; dx = 0.0; dy = 0.04733; scale = False # arcs_p05 10.25 deg
        # # da = 1.0; dx = -0.129196421; dy = 0.0; scale = True # arcs_p10 13.75 deg
        # # da = 6.0; dx = 0.0; dy = 0.0909301;scale = False # arcs_p15 17.0 deg

        # # da = 0.0; dx = 0.0; dy = 0.0;scale = False # kincs_m05 -4.0 deg
        # da = -1.0; dx = 0.0; dy = 0.0;scale = False # kincs_m10 -11.0 deg
        # # da = 3.0; dx = 0.0; dy = 0.10236;scale = False # kincs_m15 -13.5 deg
        # # da = 1.0; dx = 0.0; dy = 0.0;scale = False # kincs_p05 5.25 deg
        # # da = 1.0; dx = 0.0; dy = 0.0;scale = False # kincs_p10 9.0 deg
        # # da = 2.0; dx = 0.0; dy = 0.0571265;scale = False # kincs_p15 11.5 deg
        # x1max = np.max(x) / info["c"]
        ...
        # # multiply to have same chord
        # sc = np.max(u["out"][0]) / info["c"] / x1max
        # if scale:
        #     print("scale",sc)
        #     x1 *= sc
        #     y1 *= sc
        
        # filename = filename[:-7] + "rot.txt"

        # with open(filename,"w") as f:

        #     for i in range(x1.shape[0]):
        #         f.write("{}  {}\n".format(x1[i]/info["c"],y1[i]/info["c"]))
        ...
        plt.xlabel("x/c")
        plt.ylabel("y/c")
        # plt.xlim((-0.42152395977056667,12.421553041799836))
        # plt.ylim((-3.2919162220448714,3.733799288222808))
        plt.axis("equal")
        plt.show()
